...
import { usePutDonneeStatusMutation } from "../../app/donnees-api"
...
export default function ModalStatusDonnee() {
  /**
     * fetch donnee's id
     */
  ...
  /**
     * put donnee's status
     */
  const [putStatusDonnee, postQuery] = usePutDonneeStatusMutation()

  const changeStatus = async () => {
    try {
      await putStatusDonnee(String(id) + "/" + id2).unwrap()
      setSucces(true)
    } catch (error) {
      console.log(error);
      setNotSucces(true)
  ...
  return (
    <>
      <button onClick={() => setOpen(true)}
        className="ml-2 block p-4 text-blue-700 transition-all bg-blue-100 border-2 border-white rounded-full
                 ...
                 transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    ...
                    <div className="sm:flex sm:items-start">
                      <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <Dialog.Title as="h3" className="text-base leading-6 font-light text-gray-900 tracking-wider">
                          Modifier la donnée de status en  <strong
                  ...
                  <div className="bg-greyNakala/10 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button
                      ...
                      py-2 bg-green-100 text-base font-medium text-green-700 hover:bg-green-700 hover:text-white hover:duration-700 focus:outline-none
                    ...
                    <button
                      ...
                      className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm
                       px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 
                       focus:ring-blue-300 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"

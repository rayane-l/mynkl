...
import { usePostMetadataDonneesMutation } from "../../app/donnees-api";
...
export default function ModalAddAllMeta() {
  /**
     * gestion feedback
     */
  ...
  const [form, setForm] = useState<Metas>({ "typeUri": null } as Metas)
  const cancelButtonRef = useRef(null)

  const [postMetadataDonnee, postQuery] = usePostMetadataDonneesMutation()
  ...
  const handleSubmit = async (event: React.FormEvent<EventTarget>) => {
    ...
    try {
      await postMetadataDonnee({ id: String(id) + "/" + id2, body: form }).unwrap()
      setSuccess(true)
    } catch (error) {
      setError(true)
      console.log(error);
            ...
            <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
              ...
              >
                ...
                shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                  ...
                  <div>
                    <form action="#" method="POST" onSubmit={handleSubmit} className="p-6 mt-4  rounded-lg shadow-md">
                      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                          <label className="sr-only" htmlFor="select">Title</label>
                          <select id='select' name="propertyUri" onChange={handleChange} required
                            className='w-full p-3 text-sm border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-blue-100 "'>
                            <option value=" "> Choisissez une proprieté</option>
                            ...
                            <option value="http://purl.org/dc/terms/isPartOf">isPartOf</option>
                            <option value="http://purl.org/dc/terms/hasPart">hasPart</option>
                          </select>
                        </div>
                        <div>
                      ...
                      <div className=" px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse shadow-md rounded-lg ">
                        <button
                          ...
                          shadow-sm px-4 py-2 bg-green-100 text-base font-medium text-green-700 hover:bg-green-700 hover:text-white
                           hover:duration-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm"
                        >
                          Valider
                        </button>
                        <button
                          ...
                          className="mt-3 w-full inline-flex justify-center rounded-md border
                           border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700
                            hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-300 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
                  ...
                  <ToastSuccess closeModal={() => setSuccess(false)} message="Métadonnée ajoutée" position={true} showModal={succes} />
